...
const app = new Hono()
	.get("/", sessionMiddleware, async (c) => {
		...
		const workspaceIds = members.documents.map((el) => el.workspaceId);
	...
	.post("/", zValidator("form", createWorkspaceSchema), sessionMiddleware, async (c) => {
		...
		await databases.createDocument(
			...
			ID.unique(),
			{
				...
				role: MemberRole.ADMIN, // who created this is an admin
			}
		);
	...
	.patch(
		...
		async (c) => {
			...
			if (image instanceof File) {
				...
				uploadedImageUrl = image; // cause user might not update their image which was inserted before
			}
